...
async function fetchProductsBySupplierName(supplierID) {
    try {
        const response = await fetch(`https://www.smithsfallsnailsspa.com/api/suppliers/${supplierID}/products`);
        if (!response.ok) {
            throw new Error(`httpss error! Status: ${response.status}`);
        };
...
async function genInfoToForm() {
    ...
    try{
        const dataS = await fetch(`https://www.smithsfallsnailsspa.com/api/suppliers/search/${supplierID}`);
...
checkSave.addEventListener("click", async function (e){
    ...
    try {
        ...
        const resPOST = await fetch('https://www.smithsfallsnailsspa.com/api/imports/create', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
